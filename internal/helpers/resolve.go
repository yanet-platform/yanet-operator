...
type ResolvedComponent struct {
	...
	Numa         int32
	DisabledNuma []int32
	Networks     []yanetv1alpha1.NetworkAttachment
...
func FindBoxType(config *yanetv1alpha1.YanetConfigSpec, name string) (*yanetv1alpha1.BoxType, error) {
	if config == nil {
	...
	return nil, fmt.Errorf("boxType %q not found in YanetConfig", name)
}

// FindOperator returns a declared standalone operator.
func FindOperator(config *yanetv1alpha1.YanetConfigSpec, name string) (*yanetv1alpha1.OperatorSpec, error) {
	if config == nil {
		return nil, fmt.Errorf("yanetConfig is nil")
	}
	...
	return nil, fmt.Errorf("operator %q not found in YanetConfig.spec.components.operators", name)
}
...
func ResolveBoxComponent(config *yanetv1alpha1.YanetConfigSpec, yanet *yanetv1alpha1.YanetSpec,
	...
	case KindBirdAdapter:
		...
		if config.Components.BirdAdapter == nil {
			return nil, fmt.Errorf("boxType %q wires birdAdapter but YanetConfig.spec.components.birdAdapter is not defined", box.Name)
		}
...
func resolveDataplaneNetworks(defaults []yanetv1alpha1.NetworkAttachment, yanet *yanetv1alpha1.YanetSpec) ([]yanetv1alpha1.NetworkAttachment, []string, error) {
	if err := yanetv1alpha1.ValidateNetworkAttachments(defaults); err != nil {
		return nil, nil, err
	}
	selected := defaults
	...
	if err := yanetv1alpha1.ValidateNetworkAttachments(selected); err != nil {
		return nil, nil, err
	}
	resources := make(map[string]bool)
	for _, group := range [][]yanetv1alpha1.NetworkAttachment{defaults, selected} {
...
func ResolveBoxServiceComponent(config *yanetv1alpha1.YanetConfigSpec, boxName string,
	kind ComponentKind, operatorName string,
) (*ResolvedComponent, error) {
	return ResolveBoxComponent(config, &yanetv1alpha1.YanetSpec{BoxType: boxName}, kind, operatorName)
}
...
func EnabledComponentsForBox(config *yanetv1alpha1.YanetConfigSpec, boxName string) ([]ComponentRef, error) {
	...
	if box.Components.Dataplane != nil {
		...
		sidecars, err := resolveDataplaneSidecars(config, &yanetv1alpha1.YanetSpec{BoxType: boxName}, box)
		if err != nil {
			return nil, err
		}
		for _, sidecar := range sidecars {
...
func resolveDisabledNuma(clusterWide []int32, yanet *yanetv1alpha1.YanetSpec) []int32 {
...
func resolveDataplaneSidecars(config *yanetv1alpha1.YanetConfigSpec, yanet *yanetv1alpha1.YanetSpec,
	box *yanetv1alpha1.BoxType,
) ([]*ResolvedComponent, error) {
	palette := config.Components.Dataplane.Sidecars
	if len(palette) > yanetv1alpha1.MaxDataplaneSidecars {
	...
	for index, sidecar := range palette {
		...
		if err := yanetv1alpha1.ValidateListeners(sidecar.Name, sidecar.Listeners); err != nil {
			return nil, err
		}
		slot, wired := box.Components.Dataplane.Sidecars[sidecar.Name]
		var override *yanetv1alpha1.YanetContainerOverride
...
func resolveOperator(config *yanetv1alpha1.YanetConfigSpec, yanet *yanetv1alpha1.YanetSpec,
	box *yanetv1alpha1.BoxType, name string,
) (*ResolvedComponent, error) {
	...
	if err := yanetv1alpha1.ValidateListeners(name, operator.Listeners); err != nil {
		return nil, err
	}
...
func resolveListeners(listeners *[]yanetv1alpha1.OperatorListener) []string {
	if listeners == nil {
		return nil
	}
	result := make([]string, 0, len(*listeners))
	for _, protocol := range []yanetv1alpha1.OperatorListener{"grpc", "http"} {
...
func componentOverride(yanet *yanetv1alpha1.YanetSpec, kind ComponentKind, operatorName string) *yanetv1alpha1.YanetComponentOverride {
...
func resolveEnabled(override *yanetv1alpha1.YanetComponentOverride) bool {
...
func containerOverride(override *yanetv1alpha1.YanetComponentOverride, name string) *yanetv1alpha1.YanetContainerOverride {
	if override != nil {
