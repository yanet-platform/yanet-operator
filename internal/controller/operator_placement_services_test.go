...
func setMonitorPlacement(config *api.YanetConfig, colocated bool) {
	moveMonitorRole(&config.Spec, colocated)
}

// Move the declaration itself; placement is no longer a public API field.
func moveMonitorRole(config *api.YanetConfigSpec, colocated bool) {
	if colocated {
	...
	for _, reverse := range []bool{false, true} {
		for _, autoSync := range []bool{false, true} {
			t.Run(fmt.Sprintf("reverse=%t/autoSync=%t", reverse, autoSync), func(t *testing.T) {
				...
				yanet := reviewYanet()
				config := &api.YanetConfig{ObjectMeta: metav1.ObjectMeta{Name: "config", UID: "config-owner"}, Spec: colocatedConfig(t)}
				setMonitorPlacement(config, reverse)
				...
				r, snapshot := makeReconcilerEnv(t, yanet, second, healthy, otherInstallation, reviewNode(), config)
				...
				before := placementService(t, r, "monalive")
				...
				after := placementService(t, r, "monalive")
...
func TestOperatorPlacementServiceCutoverResidualProducers(t *testing.T) {
	for _, kind := range []string{"Deployment", "ReplicaSet", "Pod"} {
		t.Run(kind, func(t *testing.T) {
			...
			yanet := reviewYanet()
			config := &api.YanetConfig{ObjectMeta: metav1.ObjectMeta{Name: "config", UID: "config-owner"}, Spec: colocatedConfig(t)}
			setMonitorPlacement(config, false)
			...
			setMonitorPlacement(config, true)
			if err := r.Update(testContext, config); err != nil {
				t.Fatal(err)
			}
			...
			if !reflect.DeepEqual(before.Spec, placementService(t, r, "monalive").Spec) {
