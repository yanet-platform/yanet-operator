...
// also re-fetches the Yanet so callers always see the latest copy.
func reconcileTwice(t *testing.T, r *YanetReconciler, yanet *yanetv1alpha1.Yanet) {
	t.Helper()
...
func TestReconcile_DeletionTimestamp_RunsCleanupAndRemovesFinalizer(t *testing.T) {
	autoSync := true
	now := metav1.Now()
	yanet := &yanetv1alpha1.Yanet{
	...
	// owned by YanetConfig and is outside Yanet finalizer cleanup.
	...
	// All Yanet-managed resources are gone.
...
func TestPruneOrphans_DeletesUnknownDeployments(t *testing.T) {
	...
	keep.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
	drop.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
...
func TestPruneOrphans_DeletesStaleInlineConfigMap(t *testing.T) {
	...
	stale.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
	fresh.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
	...
	// Foreign Yanet's CM must not be touched.
...
// ---------------------------------------------------------------------------
// H3: UpdateWindow throttle for the path
// ---------------------------------------------------------------------------

func yanetOwnerReferenceForTest() metav1.OwnerReference {
	controller := true
...
func TestApplyDeployment_UpdateWindowThrottlesCrossHostUpdate(t *testing.T) {
	...
	state, requeue, err := r.applyDeployment(
		context.Background(),
		desired.DeepCopy(),
		true,           // autoSync
...
func TestApplyDeployment_UpdateWindowZero_AlwaysApplies(t *testing.T) {
...
func TestApplyDeployment_RejectsForeignOwner(t *testing.T) {
	existingOwner := yanetOwnerReferenceForTest()
	...
	desired.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
	desired.Labels = map[string]string{"v": "new"}
	r, _ := makeReconcilerEnv(t, existing)

	state, _, err := r.applyDeployment(
...
func TestComputeConditions_Healthy(t *testing.T) {
	y := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet", Generation: 7},
		Status: yanetv1alpha1.YanetStatus{
			Sync: yanetv1alpha1.SyncStatus{Synced: []string{"d1", "d2"}},
		},
	}
	got := computeConditions(y, nil)
...
func TestComputeConditions_OperatorMissing_Degraded(t *testing.T) {
	y := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
	}
	got := computeConditions(y, map[string]struct{}{"telegraf": {}})
	for _, c := range got {
		if c.Type == "Degraded" {
...
func TestComputeConditions_SyncWaiting_ProgressingTrue(t *testing.T) {
	y := &yanetv1alpha1.Yanet{
		Status: yanetv1alpha1.YanetStatus{
			Sync: yanetv1alpha1.SyncStatus{SyncWaiting: []string{"cp"}},
		},
	}
	got := computeConditions(y, nil)
	for _, c := range got {
...
func TestComputeConditions_PreservesLastTransitionTime(t *testing.T) {
...
func TestSetConditionsDegraded_ClearsReady(t *testing.T) {
	y := &yanetv1alpha1.Yanet{}
	setConditionsDegraded(y, "ConfigNotLoaded", "snap empty")
...
func TestCollectPods_GroupsByPhase(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
	}
	pods := []client.Object{
	...
	out, err := collectPods(context.Background(), r.Client, yanet)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReconcile_AutoSyncOn_PrunesOrphanDeployment(t *testing.T) {
	autoSync := true
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
		Spec: yanetv1alpha1.YanetSpec{
			...
			Components: &yanetv1alpha1.YanetComponentsOverride{
				Operators: map[string]yanetv1alpha1.YanetComponentOverride{"removed": {}},
			},
		},
	}
	node := &corev1.Node{
	...
	orphan.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
...
func TestReconcile_RemovesSidecarDespiteStaleOverride(t *testing.T) {
	autoSync := true
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
		Spec: yanetv1alpha1.YanetSpec{
	...
	config.Components.Dataplane.Sidecars = []yanetv1alpha1.SidecarSpec{
		{Name: "bird",
			Image: yanetv1alpha1.ImageRef{Name: "bird", Tag: "v1"},
		},
	}
	config.BoxTypes[0].Components.Dataplane.Sidecars = map[string]yanetv1alpha1.BoxDataplaneSidecar{
		"bird": {},
	}
