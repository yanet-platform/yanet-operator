...
func TestYanetConfigReconcileSerializesSnapshotReadAndPublication(t *testing.T) {
	...
	cl := interceptor.NewClient(base, interceptor.Funcs{
		Get: func(ctx context.Context, c client.WithWatch, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
			...
			cfg := obj.(*yanetv1alpha1.YanetConfig)
			if cfg.Spec.Stop {
				return nil
			}
			close(readStarted)
	...
	latest := &yanetv1alpha1.YanetConfig{}
...
func TestYanetConfigReconcilePublishesIndependentSnapshot(t *testing.T) {
	config := &yanetv1alpha1.YanetConfig{
		ObjectMeta: metav1.ObjectMeta{Name: yanetv1alpha1.YanetConfigName},
		Spec:       minimalConfig(),
	}
	...
	scheme := newSchemeForTest(t)
	var fetched *yanetv1alpha1.YanetConfig
	cl := fake.NewClientBuilder().WithScheme(scheme).WithObjects(config).
		WithInterceptorFuncs(interceptor.Funcs{
			Get: func(ctx context.Context, c client.WithWatch, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
				...
				fetched = obj.(*yanetv1alpha1.YanetConfig)
				return nil
			},
		}).Build()
	snapshot := &yanetv1alpha1.MutexYanetConfigSpec{}
...
func TestYanetConfigReconcileClearsSnapshotOnRefreshError(t *testing.T) {
	for _, readErr := range []error{errors.New("config read failed"), context.Canceled} {
		t.Run(readErr.Error(), func(t *testing.T) {
			...
			snapshot := &yanetv1alpha1.MutexYanetConfigSpec{Config: minimalConfig()}
			r := &YanetConfigReconciler{Client: cl, Scheme: scheme, GlobalConfig: snapshot}
			...
			snapshot.Lock.Lock()
			cleared := reflect.DeepEqual(snapshot.Config, yanetv1alpha1.YanetConfigSpec{})
			snapshot.Lock.Unlock()
			if !cleared {
...
func TestMapConfigToYanetsRefreshErrorPreservesSubsequentPublication(t *testing.T) {
	...
	snapshot := &yanetv1alpha1.MutexYanetConfigSpec{Config: minimalConfig()}
	configReconciler := &YanetConfigReconciler{Client: base, Scheme: scheme, GlobalConfig: snapshot}
	cl := interceptor.NewClient(base, interceptor.Funcs{
	...
	r := &YanetReconciler{Client: cl, Scheme: scheme, GlobalConfig: snapshot}
	requests := r.mapConfigToYanets(testContext, config)
