...
type defaultingObserverV2 struct {
	client.Client
	writes, dryRuns int
	dryRunError     error
	hideServices    bool
...
func TestV2APIServerDefaulting(t *testing.T) {
	...
	c := &defaultingObserverV2{Client: apiClient}
	testContext, cancelTest := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancelTest()
	config := &yanetv2alpha1.YanetConfigV2{ObjectMeta: metav1.ObjectMeta{Name: "config"}, Spec: minimalConfigV2()}
	config.Spec.Patches = nil
	yanet := reviewYanetV2()
	yanet.UID = ""
	for _, obj := range []client.Object{&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: yanet.Namespace}}, reviewNodeV2(), config, yanet} {
	...
	t.Run("patch-and-removal", func(t *testing.T) {
		config.Spec.Patches = []yanetv2alpha1.NamedPatch{{Name: "defaults", Patch: runtime.RawExtension{Raw: []byte(`{"spec":{"revisionHistoryLimit":4,"progressDeadlineSeconds":120,"template":{"spec":{"containers":[{"name":"controlplane","image":"cp:v2","resources":{"limits":{"cpu":"1"}}}]}}}}`)}}}
