...
// ---------------------------------------------------------------------------
// F23: updateStatus — RetryOnConflict on Status().Update
// ---------------------------------------------------------------------------

// TestUpdateStatus_RetriesOnConflict verifies that a transient 409
...
func TestUpdateStatus_RetriesOnConflict(t *testing.T) {
	...
	mutateCalls := 0
	err := r.updateStatus(context.Background(), yanet, func(fresh *yanetv1alpha1.Yanet) {
		mutateCalls++
...
// TestApplyDeployment_RetriesConflictAndMergesLabels: the first
// Update returns 409 Conflict; the retry must succeed AND foreign
// labels (e.g. istio sidecar injection) must survive the apply.
func TestApplyDeployment_RetriesConflictAndMergesLabels(t *testing.T) {
	...
	cl := fake.NewClientBuilder().
		...
		Build()
	r := &YanetReconciler{Client: cl, Scheme: s}
...
func TestApplyDeployment_RevalidatesOwnershipInsideRetry(t *testing.T) {
		...
		WithInterceptorFuncs(interceptor.Funcs{
			Get: func(ctx context.Context, c client.WithWatch, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
				...
				if atomic.AddInt32(&gets, 1) == 2 {
					...
					foreign := yanetOwnerReferenceForTest()
					foreign.Name = "other"
...
func TestReconcileYanet_PruneFailureReturnsErrorAndDegrades(t *testing.T) {
	autoSync := true
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{
...
// TestApplySharedService_RefusesEmptyPorts ensures the R15 guard prevents
...
func TestApplySharedService_RefusesForeignService(t *testing.T) {
	existing := &corev1.Service{
	...
	desired := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{{
				APIVersion: "yanet.yanet-platform.io/v1alpha1", Kind: "YanetConfig",
				Name: yanetv1alpha1.YanetConfigName, UID: "config-uid", Controller: &controller,
			}},
		},
		Spec: corev1.ServiceSpec{
	...
	if err := r.applySharedService(context.Background(), desired); err == nil {
		t.Fatal("a Service without the YanetConfig owner must not be taken over")
	}
	got := &corev1.Service{}
...
// TestReconcile_EmitsConfigNotLoadedEvent: when the snapshot is
// empty, reconcile must record a ConfigNotLoaded warning and back off.
func TestReconcile_EmitsConfigNotLoadedEvent(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{
...
// TestReconcile_EmitsBoxTypeNotFoundEvent: a Yanet pointing at an
// unknown boxType must surface a BoxTypeNotFound event.
func TestReconcile_EmitsBoxTypeNotFoundEvent(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{
			Name: "y", Namespace: "yanet",
			Finalizers: []string{yanetFinalizer},
		},
		Spec: yanetv1alpha1.YanetSpec{BoxType: "no-such-box"},
	}
	r, snap := makeReconcilerEnv(t, yanet)
	snap.Config = minimalConfig() // only "release" exists
	rec := events.NewFakeRecorder(8)
	r.Recorder = rec
