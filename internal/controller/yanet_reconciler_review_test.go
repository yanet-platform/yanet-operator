...
func reviewYanet() *yanetv1alpha1.Yanet {
	autoSync := true
...
func reviewNode() *corev1.Node {
	return &corev1.Node{ObjectMeta: metav1.ObjectMeta{
		Name: "test-node", Labels: map[string]string{"pool": "edge"},
	}}
}

func reviewReconcile(ctx context.Context, r *YanetReconciler, yanet *yanetv1alpha1.Yanet) (ctrl.Result, error) {
	return r.Reconcile(ctx, ctrl.Request{NamespacedName: client.ObjectKeyFromObject(yanet)})
}

func TestReconcileReview_PruneRequiresControllerOwnership(t *testing.T) {
	for _, deleting := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			owner := yanetOwnerReferenceForTest()
			oldOwner := owner
			oldOwner.UID = "previous-instance"
			foreignOwner := owner
			foreignOwner.APIVersion = "example.com/v1"
			objects := []client.Object{yanet}
			for i, refs := range [][]metav1.OwnerReference{nil, {oldOwner}, {foreignOwner}, {owner}} {
			...
			for i, object := range objects[1:] {
				...
				if i >= 6 && !apierrors.IsNotFound(err) {
...
func TestReconcileReview_SelectorChangeKeepsIncumbentClaim(t *testing.T) {
	testContext := context.Background()
	current := reviewYanet()
	incumbent := reviewYanet()
	...
	r, snapshot := makeReconcilerEnv(t, current, incumbent, workload, reviewNode())
	snapshot.Config = minimalConfig()
	result, err := reviewReconcile(testContext, r, current)
...
func TestReconcileReview_EmptySelectorValueRequiresLabel(t *testing.T) {
	testContext := context.Background()
	current := reviewYanet()
	other := reviewYanet()
	other.Name = "older"
	...
	r, snapshot := makeReconcilerEnv(t, current, other, reviewNode())
	snapshot.Config = minimalConfig()
	if result, err := reviewReconcile(testContext, r, current); err != nil || result.RequeueAfter != 0 {
...
func TestReconcileReview_DeletionWaitsForForegroundCleanup(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	now := metav1.Now()
	yanet.DeletionTimestamp = &now
	workload := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{
		...
		OwnerReferences: []metav1.OwnerReference{yanetOwnerReferenceForTest()},
...
func TestReconcileReview_FinalizerConflictIsRetried(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	now := metav1.Now()
	yanet.DeletionTimestamp = &now
	...
	r.Client = interceptor.NewClient(r.Client.(client.WithWatch), interceptor.Funcs{
		Update: func(ctx context.Context, c client.WithWatch, obj client.Object, opts ...client.UpdateOption) error {
			return apierrors.NewConflict(schema.GroupResource{Group: yanetv1alpha1.GroupVersion.Group, Resource: "yanets"}, obj.GetName(), errors.New("concurrent finalizer writer"))
		},
	})
	result, err := reviewReconcile(testContext, r, yanet)
	if err == nil && result.RequeueAfter == 0 {
...
func TestReconcileReview_StopObservedDuringWrites(t *testing.T) {
...
func TestReconcileReview_ThrottledDeploymentKeepsInlineConfig(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	r, snapshot := makeReconcilerEnv(t, yanet, reviewNode())
	snapshot.Config = minimalConfig()
	snapshot.Config.Components.Dataplane.Config = &yanetv1alpha1.ConfigSource{Inline: "old content"}
	if _, err := reviewReconcile(testContext, r, yanet); err != nil {
		t.Fatal(err)
	}
...
func TestReconcileReview_NoOpDoesNotWrite(t *testing.T) {
...
func TestReconcileReview_PodListFailureRetries(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	r, snapshot := makeReconcilerEnv(t, yanet)
	snapshot.Config = minimalConfig()
...
func TestReconcileReview_InlineConfigRefusesUnownedConfigMap(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	r, snapshot := makeReconcilerEnv(t, yanet, reviewNode())
	snapshot.Config = minimalConfig()
	snapshot.Config.Components.Dataplane.Config = &yanetv1alpha1.ConfigSource{Inline: "desired content"}
	if _, err := reviewReconcile(testContext, r, yanet); err != nil {
		t.Fatal(err)
	}
	cms := &corev1.ConfigMapList{}
	...
	if _, err := reviewReconcile(testContext, r, yanet); err == nil || !strings.Contains(err.Error(), "controller owner") {
...
func TestReconcileReview_RunningPodKeepsInlineConfig(t *testing.T) {
...
func TestReconcileReview_RenderingDoesNotMutateSnapshot(t *testing.T) {
	yanet := reviewYanet()
	r, snapshot := makeReconcilerEnv(t, yanet, reviewNode())
	snapshot.Config = sidecarConfig()
...
func TestReconcileReview_PruneRejectsConcurrentOwnershipChange(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
...
func TestReconcileReview_InlineConfigWaitsForObservedRollout(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	r, snapshot := makeReconcilerEnv(t, yanet, reviewNode())
	snapshot.Config = minimalConfig()
	snapshot.Config.Components.Dataplane.Config = &yanetv1alpha1.ConfigSource{Inline: "old config"}
	if _, err := reviewReconcile(testContext, r, yanet); err != nil {
		t.Fatal(err)
	}
	markConverged := func() {
...
func TestYanetEnvtestCleanupCompletesForegroundDeletion(t *testing.T) {
	testContext := context.Background()
	yanet := reviewYanet()
	deployment := &appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{
	...
	cm := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{
		Name: "owned-config", Namespace: yanet.Namespace, OwnerReferences: []metav1.OwnerReference{yanetOwnerReferenceForTest()},
	}}
	...
	if err := cleanupYanetResourcesForTest(testContext, r.Client, yanet.Namespace); err != nil {
