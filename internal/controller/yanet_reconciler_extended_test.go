...
// ---------------------------------------------------------------------------
// Edge cases for reconcileYanet
// ---------------------------------------------------------------------------

// TestReconcile_EmptyNodeSelector_NoDeployments verifies that when
// nodeSelector matches no nodes, reconcile succeeds but creates no
// Deployments or Services.
func TestReconcile_EmptyNodeSelector_NoDeployments(t *testing.T) {
...
// TestReconcile_ConfigNotLoaded_Requeues verifies that when
// YanetConfig snapshot is empty, reconcile sets Degraded condition
// and requeues.
func TestReconcile_ConfigNotLoaded_Requeues(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
		Spec: yanetv1alpha1.YanetSpec{
			BoxType:      "release",
	...
	// Leave GlobalConfig.Config empty to simulate config not loaded
...
// TestReconcile_BoxTypeNotFound_Requeues verifies that when the
// referenced boxType doesn't exist in YanetConfig, reconcile sets
// Degraded condition and requeues.
func TestReconcile_BoxTypeNotFound_Requeues(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
		Spec: yanetv1alpha1.YanetSpec{
	...
	snap.Config = minimalConfig() // has "release" boxType, not "non-existent"
...
// TestReconcile_StopEnabled_SkipsReconcile verifies that when
// YanetConfig.spec.stop=true, reconcile is skipped entirely.
func TestReconcile_StopEnabled_SkipsReconcile(t *testing.T) {
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{Name: "y", Namespace: "yanet"},
		Spec: yanetv1alpha1.YanetSpec{
			BoxType:      "release",
...
func TestReconcile_StopEnabled_FreezesDeletion(t *testing.T) {
	now := metav1.Now()
	yanet := &yanetv1alpha1.Yanet{
		ObjectMeta: metav1.ObjectMeta{
			Name:              "y",
	...
	if result, err := r.reconcileYanet(context.Background(), yanet); err != nil || result != (ctrl.Result{}) {
...
func TestOrphanCleanup_MultipleResourceTypes(t *testing.T) {
	...
	for _, object := range []client.Object{orphanDep, orphanCM, keepDep} {
		object.SetOwnerReferences([]metav1.OwnerReference{yanetOwnerReferenceForTest()})
	}
	...
	if err := r.Client.Get(context.Background(), types.NamespacedName{Name: "orphan-svc", Namespace: "yanet"}, &corev1.Service{}); err != nil {
		t.Errorf("Service must be left for YanetConfig reconciliation: %v", err)
	}
...
// desired set is empty (e.g., during deletion), all Yanet-owned resources
// are deleted while Services are left for YanetConfig reconciliation.
func TestOrphanCleanup_EmptyDesiredSet_DeletesAll(t *testing.T) {
	...
	dep1.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
	dep2.OwnerReferences = []metav1.OwnerReference{yanetOwnerReferenceForTest()}
