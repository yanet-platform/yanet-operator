...
func sidecarConfigV2() api.YanetConfigSpec {
...
func TestReconcileV2_BirdConsumersHaveNoNameBasedDependency(t *testing.T) {
	for _, bird := range []bool{true, false} {
		t.Run(fmt.Sprint(bird), func(t *testing.T) {
			...
			for _, role := range []string{"birdAdapter", "announcer"} {
				deployment := transitionDeploymentV2(t, r, role)
...
func TestReconcileV2_DisabledSidecarReservedTarget(t *testing.T) {
	for _, mode := range []string{"box-disabled", "override-disabled"} {
		t.Run(mode, func(t *testing.T) {
			...
			target := manifests.BuildServices(manifests.BuildContextV2{BoxType: "release"}, component)[0].Ports[0].TargetPortName
			...
			r, snapshot := makeReconcilerEnv(t, yanet, reviewNodeV2())
			snapshot.Config = config
			if _, err := reviewReconcileV2(context.Background(), r, yanet); err == nil || !strings.Contains(err.Error(), target) {
...
func TestReconcileV2_DisabledSidecarServiceStatus(t *testing.T) {
	for _, noNodes := range []bool{false, true} {
		for _, mode := range []string{"enabled", "box-disabled", "override-disabled", "dataplane-disabled", "installation-disabled", "unwired"} {
			t.Run(fmt.Sprintf("%s/noNodes=%t", mode, noNodes), func(t *testing.T) {
				...
				yanet := reviewYanetV2()
				config := &api.YanetConfigV2{ObjectMeta: metav1.ObjectMeta{Name: api.YanetConfigName, UID: "config-owner"}, Spec: sidecarConfigV2()}
				switch mode {
				case "box-disabled":
				...
				if !noNodes {
					deployment := transitionDeploymentV2(t, r, "dataplane")
					hasNeighbour := false
