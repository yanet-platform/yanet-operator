...
// ValidatePrivatePodNetwork enforces private networking after all patches.
func ValidatePrivatePodNetwork(pod *corev1.PodSpec) error {
	if pod.HostNetwork {
		return fmt.Errorf("a private network namespace is required; hostNetwork is unsupported")
	}
	for _, containers := range [][]corev1.Container{pod.Containers, pod.InitContainers} {
		for _, container := range containers {
			for _, port := range container.Ports {
				if port.HostPort != 0 {
					return fmt.Errorf("container %q must not declare hostPort %d", container.Name, port.HostPort)
				}
			}
		}
	}
